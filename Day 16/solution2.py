...
result = visit_from_valve('AA', 26, 0, 0, {})
best_result = max([result[x] + result[y] for x in result for y in result if not x & y])
end = time.perf_counter()
print(f'Trabajando en conjunto con el elefante, la mayor cantidad posible de presión a liberar'
      f' es: {print_color(best_result, Color.GREEN)}')
