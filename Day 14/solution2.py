...
max_y = max_y + 1 + PAD
min_y -= PAD
cave = [[AIR for x in range(max_y)] for y in range(max_x + 1)]
cave.append([AIR for x in range(max_y)])
cave.append([ROCK for x in range(max_y)])
put_rocks()

sand_resting_count = 0
cave_has_space_left = True
try:
    while cave_has_space_left:
        ...
        while True:
            ...
            if new_sand_position == initial_sand_position:
                ...
                if new_sand_position == SAND_STARTING_POSITION:
                    cave_has_space_left = False
                break
