...
with open('input.txt', 'r') as file:
    for line in file:
        ...
        if reading_stacks:
            ...
            _, crates_to_move, _, stack_from, _, stack_to = line.strip().split()

            crates_to_move = int(crates_to_move)
            remaining_crates = len(stacks[stack_from]) - crates_to_move
            stacks[stack_to] += stacks[stack_from][-crates_to_move:]
            stacks[stack_from] = stacks[stack_from][0:remaining_crates]
